...
- **`vercel.json`**: `trailingSlash: false` evita brigas de redirect com o Fastify. **`/simulator`** reescreve para **`/simulator.html`** (estático). O resto vai para **`/api/$1`**. O prefixo **`/api`** é removido **só** em **`api/[[...path]].js`** (`req.url` antes do `serverless-http`), sem usar headers (`x-forwarded-path`, etc.) — isso evita URLs inconsistentes e **loops de redirect** na edge. `ignoreTrailingSlash: true` no Fastify alinha rotas com ou sem barra final.
