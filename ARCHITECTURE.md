...
- **`vercel.json`**: `trailingSlash: false` evita brigas de redirect com o Fastify. **`/simulator`** reescreve para **`/simulator.html`** (ficheiro estático em `public/`) para **não** invocar a Serverless Function — evita cold start de vários segundos só para abrir o HTML. O resto do tráfego vai para `/api/$1` como antes. Em `VERCEL=1`, um hook repõe o path pelos headers e remove o prefixo `/api` para `/webhook`, `/files`, etc. `ignoreTrailingSlash: true` no Fastify alinha rotas com ou sem barra final.
