...
- **`vercel.json`**: `trailingSlash: false` evita brigas de redirect com o Fastify. **`/simulator`** e **`/simulator/`** reescrevem para **`/simulator.html`**. O resto vai para **`/api/$1`**. O prefixo **`/api`** é removido **só** em **`api/[[...path]].js`** (`req.url` antes do `serverless-http`). `ignoreTrailingSlash: true` no Fastify alinha rotas com ou sem barra final.
