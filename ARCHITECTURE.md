...
- **`vercel.json`**: `trailingSlash: false` evita brigas de redirect com o Fastify; `rewrites` mapeiam `/*` para `/api/$1` (sem duplicar rotas que já começam por `/api`). Em `VERCEL=1`, um hook repõe o path pelos headers (URLs absolutas em `x-url` são convertidas para pathname) e **sempre** remove o prefixo `/api` para bater com `/webhook`, `/simulator`, etc. `ignoreTrailingSlash: true` no Fastify alinha `/simulator` com `/simulator/`.
