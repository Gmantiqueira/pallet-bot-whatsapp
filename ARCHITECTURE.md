...
- **Simulador estático**: o `npm run build` corre `scripts/sync-simulator-static.cjs`, que lê **`public/simulator.source.html`** e gera **`public/simulator.html`** (`.gitignore`, com secret injetado). Na Vercel, preset “Other” expõe **`public/*` na raiz do site**, por isso **`/simulator.html`** existe como estático; gerar `simulator.html` só na raiz do repositório **não** entrava nessa camada e originava **404** em **`GET /simulator`**. O script também injeta **`WEBHOOK_SECRET`** (visível no “view source” — só para simulador interno).
